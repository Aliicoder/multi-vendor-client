...
import { otpValidation } from "@/validations/addressValidation";
import {
  ...
  async function onSubmitOtp(values: z.infer<typeof otpValidation>) {
    try {
      const response = await verifyEmailOtpMutation({
        ...
        requestedRole: userForm.getValues("requestedRole"),
      }).unwrap();
      successToast(response.message);
      navigate("/login");
    } catch (error: any) {
        ...
        <Form {...userForm}>
          ...
          >
            ...
            <div className="flex flex-col w-full gap-2 ">
              <CustomButton form="signup-form" theme="black" className="">
                Signup
              </CustomButton>
              <GoogleSignInButton />
            </div>
